...
RSpec.configure do |config|
  ...
  config.before(:each) do
    ...
    %w[eth-kovan eth-rinkeby eth-rinkeby-testnet btc-testnet].each { |blockchain| FactoryBot.create(:blockchain, blockchain) }
    %i[usd eur btc eth trst ring].each { |ccy| FactoryBot.create(:currency, ccy) }
    %i[usd_network eur_network btc_network eth_network eth_testnet_network trst_network ring_network].each do |blockchain_currency|
